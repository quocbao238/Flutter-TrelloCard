  ...
  void _doPan(double xPos, double yPos) {
    ...
    for (int axisIndex = 0;
      ...
      if (_chart.onZooming != null) {
        ...
        _zoomEvent(_chart, axis, zoomingArgs, _chart.onZooming);
      }
    }
    _createZoomState();
  }
...
//Below method is for pinch zooming
  ...
  void _createZoomState() {
    ...
    _chart._chartState._isLegendToggled = false;
    ...
    for (int axisIndex = 0;
      ...
      if ((axis._orientation == AxisOrientation.vertical &&
          ...
          (axis._orientation == AxisOrientation.horizontal &&
              zoomMode != ZoomMode.y)) {
        cumulative = math.max(
            math.max(1 / _minMax(axis._zoomFactor, 0, 1), 1) +
                (0.25 * direction),
            1);
        if (cumulative >= 1) {
          origin = axis._orientation == AxisOrientation.horizontal
              ? mouseX / _chart._chartAxis._axisClipRect.width
              : 1 - (mouseY / _chart._chartAxis._axisClipRect.height);
          origin = origin > 1 ? 1 : origin < 0 ? 0 : origin;
          zoomFactor = (cumulative == 1) ? 1 : _minMax(1 / cumulative, 0, 1);
          zoomPosition = (cumulative == 1)
              ? 0
              : axis._zoomPosition + ((axis._zoomFactor - zoomFactor) * origin);
          if (axis._zoomPosition != zoomPosition ||
              axis._zoomFactor != zoomFactor) {
  ...
  void reset() {
    for (int index = 0;
      ...
      axis._zoomPosition = 0.0;
      if (_chart.onZoomReset != null) {
        ZoomPanArgs zoomResetArgs;
        _zoomEvent(_chart, axis, zoomResetArgs, _chart.onZoomReset);
      }
    }
    _chart._chartState._isLegendToggled = false;
    _chart._chartState._redraw();
